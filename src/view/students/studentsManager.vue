    ...
    <div class="container">
      ...
      <div class="pagination">
        <router-link :to="{path: $route.path.replace(/manager/g, 'detail')}">
          <el-button class="pull-left" type="success" v-permission="'student:edit'">添加</el-button>
        </router-link>
        <el-pagination
...
<script>
  ...
  export default {
    ...
    methods: {
      getData () {
        this.$axios.get('/rebuild/student/list', {params: this.queryParams}).then((res) => {
          if (res && res.data) {
      ...
      changeStatus (id, status) {
        this.$axios.post('/rebuild/student/disable', {
          id: id,
          disable: status
        }).then(res => {
          if (res && res.code === 0) {
      ...
      remove (id) {
        this.$confirm('确定删除吗？', '提示', {
          ...
          this.$axios.get('/rebuild/student/delete', {params: {id: id}}).then((res) => {
            if (res && res.code === 0) {
              this.getData()
              this.$message({
                type: 'info',
